---
name: opus-advisor
description: Independent analysis, investigation, or solution design on an arbitrary task, using Claude's Opus tier for its judgment. Read-only — never edits files. Invoked only by the `opus` skill, not typically triggered directly by user phrasing.
model: opus
