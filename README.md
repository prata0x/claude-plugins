...
| `writing` | 技術文書・開発成果物(commit/PR/issue)・changelog・議事録・翻訳それぞれの執筆ガイドスキル(いずれもAI生成っぽい定型表現を防ぐチェックを内蔵)。加えて*.md編集前のcontext注入hook、定型フレーズを検出するpost-edit機械的hook、辞書では拾えない意味的な問題(書き手の不在・構造的単調さ・定型表現の言い換え)を確認する不定期実行の監査スキル | `claude plugin install writing@prata0x-plugins` |
| `blog-pipeline` | AI執筆ブログの英語執筆→自動レビュー→日本語訳→自動レビューのパイプライン。サイトごとの`PERSONA.md`で架空キャラクターの声を差し替え可能にした汎用設計。文体レビューは`writing`プラグインの`ai-writing-scanner`/`ai-writing-confidence-filter`を1記事単位で再利用し、技術的正確性・翻訳忠実性は新規の`fact-checker`エージェントが担当 | `claude plugin install blog-pipeline@prata0x-plugins` |
